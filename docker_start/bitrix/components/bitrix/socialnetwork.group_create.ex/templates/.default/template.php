...
{
	CJSCore::Init(array('socnetlogdest', 'popup'));
...
//		)
	...
	{
		?><div class="sonet-group-create-popup-error-block">
			...
			<div class="sonet-group-create-popup-content"><?=$arResult["ErrorMessage"]?></div>
...
// this situation is impossible now but this code may be needed in the future
		...
		<form method="post" name="sonet_group_create_popup_form" id="sonet_group_create_popup_form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
			<div id="sonet_group_create_popup" class="sonet-group-create-popup"><?
				...
				<div id="sonet_group_create_tabs_content" class="sonet-group-create-tabs-content<?=($arResult["bExtranetInstalled"] ? " sonet-group-create-tabs-content-extranet" : "")?><?=($arResult["TAB"] == "invite" ? " sonet-group-create-tabs-content-invite" : "")?><?=(strlen($arResult["ErrorMessage"]) > 0 ? " sonet-group-create-tabs-content-error" : "")?>"><?
						...
						?><div id="sonet_group_create_tabs_invite"<?=($arResult["TAB"] != "invite" ? ' style="display: none;"' : '')?>><?
							...
							?><div class="sonet-group-create-popup-users-title"><?=GetMessage("SONET_GCE_T_DEST_TITLE_".(IsModuleInstalled("intranet") ? "EMPLOYEE" : "USER"))?></div><?
								?><div class="feed-add-post-destination-wrap<?=(in_array("USERS", $arResult["ErrorFields"]) ? " sonet-group-create-tabs-text-error" : "")?>" id="sonet_group_create_popup_users_container_post_<?=$selectorName?>">
									...
									$arStructure = (IsModuleInstalled('intranet') ? CSocNetLogDestination::GetStucture(array("LAZY_LOAD" => true)) : false);
									...
									?><script type="text/javascript">
										...
										BX.ready(function() {
											BX.SocNetLogDestination.init({
												'name' : '<?=$selectorName?>',
												'searchInput' : BX('sonet_group_create_popup_users_container_post_<?=$selectorName?>'),
												...
												'items' : {
													'users' : <?=(
														...
														&& strlen(COption::GetOptionString("extranet", "extranet_site")) > 0
															? (is_array($arResult["POST"]["USERS_FOR_JS_I"]) ? CUtil::PhpToJSObject($arResult["POST"]["USERS_FOR_JS_I"]) : '{}')
															: (is_array($arResult["POST"]["USERS_FOR_JS"]) ? CUtil::PhpToJSObject($arResult["POST"]["USERS_FOR_JS"]) : '{}')
													)?>,
													'groups' : {},
													'sonetgroups' : {},
								...
								)
								{
									...
									?><div id="INVITE_EXTRANET_block" style="display: <?=($arResult["POST"]["IS_EXTRANET_GROUP"] == "Y" ? "block" : "none")?>;"><?
												...
												?><script type="text/javascript"><?
													...
													BX.ready(function() {
														BX.SocNetLogDestination.init({
															...
															'items' : {
																'users' : <?=(is_array($arResult["POST"]["USERS_FOR_JS_E"]) ? CUtil::PhpToJSObject($arResult["POST"]["USERS_FOR_JS_E"]) : '{}')?>,
																'groups' : {},
																'sonetgroups' : {},
												...
												?><table class="invite-dialog-inv-form-table">
													<tr>
														...
														<td class="invite-dialog-inv-form-r">
															<textarea rows="5" type="text" name="EMAILS" id="EMAILS" class="invite-dialog-inv-form-textarea"><?=htmlspecialcharsbx($arResult["POST"]["EMAILS"])?></textarea>
														</td>
													</tr>
												</table><?
											?></div><?
										...
										?><div id="sonet_group_create_popup_action_block_invite_2" style="display: block;"><?
											...
											<textarea rows="5" type="text" name="MESSAGE_TEXT" id="MESSAGE_TEXT" class="invite-dialog-inv-form-textarea" style="width: 500px;"><?
